...
const registerValidatorSchema = z.object({
    name: z
        ...
        .max(255, { message: "Username must not be more than 255 charaters " }),
    email: z
        .string({ required_error: "Email is required" })
        .trim()
        .email({ message: "Invalid email address" })
        .min(3, { message: "Email must be at least of 3 character " })
        .max(255, { message: "Email must not be more than 255 charaters " }),
    phone: z
    ...
    password: z
        ...
        .max(255, { message: "Password must not be more than 255 charaters " }),
    role: z
